function [mean_ci, mean_Q] = Simu_Expe(congr, alphas, beta, n_it, n_trials)

    Alpha = alphas; 
    Beta = beta;
    parameters=[Beta, Alpha];

    percent_correct = zeros(2*n_it, 6); %stores % of correct choices
    ...
    for iteration = 1:n_it
        ...
        for block = 1:6
            ...
            if percent_correct(iteration, 2*structure(1, block)+structure(2, block)+1)==0
                percent_correct(iteration, 2*structure(1, block)+structure(2, block)+1) = 100*(c_correct/n_trials);
            else
                percent_correct(iteration+n_it, 2*structure(1, block)+structure(2, block)+1) = 100*(c_correct/n_trials);
            end
            
            %store Q_values in the corresponding matrix
            Q_free = Q_free(:,1:n_trials);
            if mean(Q_values([iteration iteration+2*n_it], (2*structure(1, block)+structure(2, block))*n_trials+1:(2*structure(1, block)+structure(2, block)+1)*n_trials))==0
                Q_values([iteration iteration+2*n_it], (2*structure(1, block)+structure(2, block))*n_trials+1:(2*structure(1, block)+structure(2, block)+1)*n_trials) = Q_free;
            else
                Q_values([iteration+n_it iteration+3*n_it], (2*structure(1, block)+structure(2, block))*n_trials+1:(2*structure(1, block)+structure(2, block)+1)*n_trials) = Q_free;
            end
    ...
    ci_pc = 1.96*std(percent_correct)/sqrt(2*n_it);
    
    mean_ci = [mean_pc ci_pc];
    
    mean_Q1 = mean(Q_values(1:2*n_it, :));
    mean_Q2 = mean(Q_values(2*n_it+1:4*n_it, :));
